...
def process_video_to_csv(video_path, output_csv):
    """
    Processes an existing video file and saves keypoints to CSV.
    Fixed version with proper error handling and debug logging.
    """
    try:
        # Create VideoProcessor instance
        processor = VideoProcessor(accuracy=0.3)

        print(f"\nStarting keypoint analysis...")
        ...
        # Prepare CSV with immediate flushing
        with open(output_csv, mode='w', newline='', encoding='utf-8', buffering=1) as file:
            writer = csv.writer(file)
            ...
            while True:
                ...
                print(f"\rProcessing frame {frame_idx + 1}/{frame_count} ({progress:.1f}%)", end="")
                ...
                try:
                    ...
                    if current_data is not None:
                        expected_length = 1 + (33 * 2) + (42 * 2) + (468 * 2)
                        if len(current_data) == expected_length:
                            writer.writerow(current_data)
                            file.flush()  # Force write to disk
                            processed_frames += 1
                        else:
                            print(f"\nWarning: Invalid data length for frame {frame_idx + 1}")
                            print(f"Expected {expected_length} values, got {len(current_data)}")
                    ...
                    print(f"\nError processing frame {frame_idx + 1}: {str(e)}")
        ...
        if os.path.exists(output_csv):
            ...
            print(f"Success rate: {(processed_frames / frame_count) * 100:.1f}%")
            return True
        else:
...
def main():
    ...
    try:
        ...
        for folder in [video_folder, csv_folder]:
            if not os.access(folder, os.W_OK):
                print(f"Error: No write permission for directory: {folder}")
                return
        ...
        # Process video and create CSV
        csv_path = os.path.join(csv_folder, "live_dataset.csv")

        print("\nStarting video processing...")
        print(f"Video path: {video_path}")
        print(f"CSV output: {csv_path}")

        if process_video_to_csv(video_path, csv_path):
